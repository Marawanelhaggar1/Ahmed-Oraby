...
export class ServiceService extends UnsubscribeOnDestroyAdapter {
  ...
  isTblLoading = true;
  dataChange: BehaviorSubject<ServiceModel[]> = new BehaviorSubject<
    ServiceModel[]
  >([]);
  // Temporarily stores data from dialogs
  dialogData!: ServiceModel;
  constructor(private httpClient: HttpClient) {
    super();
  }
  get data(): ServiceModel[] {
  ...
  getAllDoctorss(): void {
    this.subs.sink = this.httpClient
      .get<{ data: ServiceModel[] }>(
        'https://hws1.axonbi.com/public/api/service/admin?lang=admin'
      )
      .subscribe({
        next: (data) => {
  ...
  addDoctors(doctors: ServiceModel): void {
  ...
  updateDoctors(centers: ServiceModel): void {
