...
export class ServiceService extends UnsubscribeOnDestroyAdapter {
  ...
  isTblLoading = true;
  dataChange: BehaviorSubject<PromotionModel[]> = new BehaviorSubject<
    PromotionModel[]
  >([]);
  // Temporarily stores data from dialogs
  dialogData!: PromotionModel;
  constructor(private httpClient: HttpClient) {
    super();
  }
  get data(): PromotionModel[] {
  ...
  getAllDoctorss(): void {
    this.subs.sink = this.httpClient
      .get<{ data: PromotionModel[] }>(
        'https://hws1.axonbi.com/public/api/promotion?lang=admin'
      )
      .subscribe({
        next: (data) => {
  ...
  addDoctors(doctors: PromotionModel): void {
  ...
  updateDoctors(centers: PromotionModel): void {
